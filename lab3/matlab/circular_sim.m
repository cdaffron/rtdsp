...
audio_in = 0.5 * sin(2 * pi * signal_freq * time );
...
for j=1:length(audio_in)
    working_data(sampleInd) = audio_in(j);
    output = 0;
    
    indLeft = sampleInd;
    indRight = sampleInd + 1;
%     disp('Initial indLeft');
%     disp(indLeft);
    sampleInd = sampleInd + 1;
    if( sampleInd > length(working_data) )
        sampleInd = 1;
    end
    
    if( indRight > length(working_data) )
        indRight = 1;
    end
    
    for i=1:(length(working_data)/2)
        output = output + (working_data(indLeft) * coeffs(i)) + (working_data(indRight) * coeffs(i));
%         disp('New Iteration')
%         disp(indLeft)
%         disp(indRight)
%         disp(i)
        
%         disp(sprintf('Iteration: %d, L = %d, R = %d', i, indLeft, indRight))
        indLeft = indLeft - 1;
        if(indLeft < 1)
            indLeft = length(working_data);
        end
        indRight = indRight + 1;
        if(indRight > length(working_data))
            indRight = 1;
    ...
    if mod(N,2) == 0
        output = output + (working_data(indLeft) * coeffs(floor(N+2)/2));
    end
